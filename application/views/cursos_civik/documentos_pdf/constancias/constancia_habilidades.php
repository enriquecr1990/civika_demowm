...
<?php foreach ($Constancia as $index => $datos_cons): ?>
    ...
    <table style="font-size: 21px; padding-top: 120px !important; min-height: 100px; max-height: 100px; text-align: center"
           class="negrita contenido_const w100">
        <tr>
    ...
    <table style="min-height: 70px; max-height:70px; font-size: 40px; text-align: center"
           class="w100 nombre_alumno negrita">
        <tr>
    ...
    <table style="min-height: 50px; max-height:50px; text-align: center; font-size: 21px"
           class="w100 contenido_const negrita">
        <tr>
    ...
    <table style="line-height:.8em; font-size: <?= $font_size ?>; text-align: center" class="w100 nombre_curso negrita">
        <tr>
            <td style="vertical-align: middle; padding-left: 22px;padding-right:  22px;">"<?= $datos_cons->nombre_ctn ?>
                "
            </td>
        </tr>
    </table>

    <table style="min-height: 40px; max-height:40px; vertical-align:middle; font-size: 19px; text-align: center"
           class="w100 contenido_const negrita">
        <tr>
            <td>Impartido el <?php echo fechaCastellano($datos_cons->fecha_fin); ?>, en el aula de capacitación del
                plantel Cívika 90342.
            </td>
        </tr>
        <tr>
    ...
    <table style="min-height: 40px; max-height:40px; vertical-align:middle; font-size: 19px; text-align: center"
        ...
        <tr>
            <td class="w15"></td>
            <td style="text-align: center" class="w45">Catálogo nacional de
                Ocupaciones: <?= $datos_cons->clave_ocupacion_especifica . ' ' . $datos_cons->ocupacion_especifica ?>.
            </td>
            <td style="text-align: left">Área
                Temática: <?= $datos_cons->clave_area_tematica . ' ' . $datos_cons->Denominacion_Curso ?>.
            </td>
        </tr>
    </table>

    <table style="min-height: 40px; max-height:40px; vertical-align:middle; font-size: 14px; text-align: center"
           class="w100 contenido_const negrita">
        <tr>
            <td>Los datos se asientan en esta constancia bajo protesta de decir verdad, apercibidos de la
                responsabilidad en que incurre todo aquel que no se conduce con verdad.
            </td>
        </tr>
    </table>

    <table style="min-height: 20px; max-height:20px; vertical-align:middle; font-size: 14px; text-align: center"
           class="w100 contenido_const negrita">
        <tr>
            <td class="w13"></td>
            <td style="text-align: left">Agente capacitador</td>
            <td style="text-align: left" class="w58">Por la Comisión Mixta de Capacitación, Adiestramiento y
                Productividad
            </td>
        </tr>
    </table>
    ...
    <table style="min-height: 20px; max-height:20px; font-size: 14px; font-weight: bold" class="w100">
        <tr>
            <td class="w5"></td>
            <td class="contenido_const w26" style="text-align: center">Instructor</td>
        ...
        <tr>
            <td class="w5"></td>
            <td class="contenido_const w26"
                style="text-align: center"><?= strMayusculas($datos_cons->Nombre_Instructor) ?></td>
            <td class="contenido_const w26"
                style="text-align: center"><?= strMayusculas($datos_cons->representante_legal) ?> </td>
            <td class="contenido_const w26"
                style="text-align: center"><?= strMayusculas($datos_cons->representante_trabajadores) ?></td>
            <td class="w10"></td>
        </tr>
    </table>
    ...
    <table class="w100">
        <tr>
            <td class="derecha folio folio">Folio: <?= $datos_cons->folio_habilidades ?></td>
            <td class="w3"></td>
        </tr>
    </table>
