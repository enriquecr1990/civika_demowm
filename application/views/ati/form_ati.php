<div class="card card-primary">
	<div class="card-header">
		<span class="card-title"><?=isset($ec_instrumento_has_actividad) ? 'Modificar':'Agregar'?> intrumento de evaluación</span>
	</div>
	<div class="card-body">
		<form id="form_agregar_modificar_ec_ati">

			<input type="hidden" name="id_ec_instrumento_has_actividad" value="<?=isset($id_ec_instrumento_has_actividad) ? $id_ec_instrumento_has_actividad : ''?>">

			<div class="modal-body">
				<div class="form-group row">
					<label for="cat_instrumento" class="col-sm-3 col-form-label">Instrumento</label>
					<div class="col-sm-9">
						<select id="cat_instrumento" class="custom-select form-control-border slt_mostrar_ocultar"
								...
								data-contenedor_detalle="#descripcion_otro_instrumento" data-input_detalle="#det_cat_instrumento" data-id_show="99999"
								name="id_cat_instrumento" required>
							<option value="">-- Seleccione --</option>
							<?php if(isset($cat_instrumento)): ?>
								<?php foreach ($cat_instrumento as $ct): ?>
									<option value="<?=$ct->id_cat_instrumento?>" <?=isset($estandar_competencia_instrumento) && $estandar_competencia_instrumento->id_cat_instrumento == $ct->id_cat_instrumento ? 'selected="selected"':''?> ><?=$ct->nombre?></option>
								<?php endforeach; ?>
							<?php endif; ?>
							<!--<option value="99999">Otro</option>-->
						</select>
						<?php if(isset($estandar_competencia_instrumento)): ?>
							<input type="hidden" name="id_cat_instrumento" value="<?=$estandar_competencia_instrumento->id_cat_instrumento?>">
						<?php endif; ?>
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<input type="text" data-rule-required="true" class="form-control col-sm-9" placeholder="Describa la actividad"
							...
							<div class="col-sm-12">
								<div class="form-group row">
									<input id="input_url_video" type="url" data-rule-url="true" class="form-control col-sm-9" placeholder="URL del video"
										   value="<?=isset($ec_instrumento_actividad) ? $ec_instrumento_actividad->actividad : '' ?>">
									<div class="col-sm-3">
						...
						<?php if(isset($ec_instrumento_actividad_has_archivo)): ?>
							<?php foreach ($ec_instrumento_actividad_has_archivo as $av): ?>
								...
								<li class="mb-1" style="list-style: none">
									<?=$input?>
									<a href="<?=$href?>" target="_blank" class="btn btn-sm btn-outline-success" data-toggle="tooltip" title="<?=$nombre?>">
										<i class="<?=$icon?>"></i> <?=substr($nombre,0,20).'...'?>
									</a>
	...
	<div class="card-footer">
		<button type="button" class="btn btn-sm btn-default" id="btn_cancelar_form_ati" >Cancelar</button>
